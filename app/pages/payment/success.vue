...
onMounted(async () => {
  try {
    if (gateway === "hyperpay" && id) {
      ...
      console.log("HyperPay data:", data);

      if (data?.status === false || data?.status === "failed") {
        navigated.value = true;
        navigateTo(`/payment/failure?gateway=hyperpay&order_id=${orderId}`);
        return;
      } else {
        cartCount.value = 0;
      }
      ...
      if (pid) {
        ...
        if (data?.status === false || data?.status === "failed") {
          navigated.value = true;
          navigateTo(`/payment/failure?gateway=tabby&order_id=${orderId}`);
          return;
        } else {
          cartCount.value = 0;
        }
    ...
    if (!error.value) {
      ...
      setTimeout(() => {
        navigateTo(`/payment?order_id=${orderId}`);
      }, 3000);
    }
  }
});
