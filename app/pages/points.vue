...
<template>
    ...
    <div class="points">
        <div class="container mx-auto px-4">
                ...
                <div class="w-full max-w-4xl">
                    ...
                    <div class="mb-6">
                        <ul class="flex items-center justify-center gap-8 border-b border-gray-200 pb-3 capitalize">
                            <li @click="activeTab = 'earned'" :class="['cursor-pointer pb-2 font-semibold transition', activeTab === 'earned' ? 'border-b-2 border-black text-black' : 'text-gray-500 hover:text-black']">
                                Earned
                            </li>
                            <li @click="activeTab = 'redeem'" :class="['cursor-pointer pb-2 font-semibold transition', activeTab === 'redeem' ? 'border-b-2 border-black text-black' : 'text-gray-500 hover:text-black']">
                                Redeem
                            </li>
                            <li @click="activeTab = 'expired'" :class="['cursor-pointer pb-2 font-semibold transition', activeTab === 'expired' ? 'border-b-2 border-black text-black' : 'text-gray-500 hover:text-black']">
                                Expired
                            </li>
                        </ul>
                    </div>
                    ...
                    <template v-else>
                        ...
                        <div v-for="item in filteredTransactions" :key="item.id"
                            ...
                            <div class="flex items-center gap-4">
                                <div :class="['flex h-12 w-12 items-center justify-center rounded-full', item.type === 'credit' ? 'bg-green-100' : activeTab === 'expired' ? 'bg-red-100' : 'bg-yellow-100']">
                                    <svg v-if="item.type === 'credit'" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M14.6673 6.7487H9.25065V1.33203C9.25065 0.648698 8.68398 0.0820312 8.00065 0.0820312C7.31732 0.0820312 6.75065 0.648698 6.75065 1.33203V6.7487H1.33398C0.650651 6.7487 0.0839844 7.31536 0.0839844 7.9987C0.0839844 8.68203 0.650651 9.2487 1.33398 9.2487H6.75065V14.6654C6.75065 15.3487 7.31732 15.9154 8.00065 15.9154C8.68398 15.9154 9.25065 15.3487 9.25065 14.6654V9.2487H14.6673C15.3507 9.2487 15.9173 8.68203 15.9173 7.9987C15.9173 7.31536 15.3507 6.7487 14.6673 6.7487Z" fill="#41C980" />
                                    </svg>
                                    <svg v-else width="24" height="4" viewBox="0 0 24 4" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M22 3.25H2C1.31667 3.25 0.75 2.68333 0.75 2C0.75 1.31667 1.31667 0.75 2 0.75H22C22.6833 0.75 23.25 1.31667 23.25 2C23.25 2.68333 22.6833 3.25 22 3.25Z" fill="#FDD800" />
                                    </svg>
                                </div>
                                <div>
                                    <h4 class="font-semibold text-gray-900">{{ item.type_value ?? (item.type === 'credit' ? 'Earned Points' : 'Points Used') }}</h4>
                                    <p class="text-sm text-gray-500">{{ formatDate(item.created_at ?? item.date) }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                <span :class="['text-xl font-bold', item.type === 'credit' ? 'text-green-600' : 'text-gray-900']">{{ pointsSign(item) }}{{ item.points?.replace(/[+\-]/g, '') ?? item.points }}</span>
            ...
            <!-- Action Sheet -->
            <Teleport to="body">
                <div v-if="showActionSheet" @click.self="showActionSheet = false" class="fixed inset-0 z-50 flex items-end justify-center bg-black/40 backdrop-blur-sm">
                    <div class="w-full max-w-md rounded-t-3xl bg-white p-6 shadow-2xl">
                        <div class="mb-5 flex items-center justify-between">
                            <h2 class="text-lg font-semibold">Send Points</h2>
                            <button @click="showActionSheet = false" class="flex h-9 w-9 items-center justify-center rounded-full bg-gray-100">
            ...
            <!-- Redeem Modal -->
            <Teleport to="body">
                <div v-if="showRedeemModal" @click.self="showRedeemModal = false" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
                    <div class="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
                        <div class="mb-5 flex items-center justify-between">
                            <h2 class="text-xl font-semibold text-gray-900">Redeem Points</h2>
                            <button @click="showRedeemModal = false" class="flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200">
                        ...
                        <div>
                            ...
                            <p class="mt-2 text-sm text-gray-500">Notes: Minimum points = {{ exampleRate.points }} points</p>
                        </div>
                        <div class="mt-5 flex items-center justify-between rounded-2xl bg-gray-50 p-4">
                            ...
                            <div class="text-center">
                                <h6 class="text-xl font-bold text-gray-900">{{ Math.floor(Number(redeemAmount || 0) / exampleRate.points * exampleRate.amount) }}</h6>
                                <p class="text-sm uppercase text-gray-500">SAR</p>
                            </div>
                        </div>
                        <button @click="handleRedeem" :disabled="loadingRedeem || !redeemAmount || Number(redeemAmount) < exampleRate.points"
                            ...
                            <span v-if="loadingRedeem" class="h-5 w-5 border-2 border-white border-t-transparent rounded-full animate-spin"></span>
            ...
            <Teleport to="body">
                <div v-if="showTransferModal" @click.self="showTransferModal = false" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
                    <div class="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
                        <div class="mb-5 flex items-center justify-between">
                            <h2 class="text-xl font-semibold text-gray-900">Send Points</h2>
                            <button @click="showTransferModal = false" class="flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200 transition">
                                <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18M6 6l12 12"/></svg>
                            </button>
                        </div>
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input v-model="transferPoints" type="number" :placeholder="transferInfo?.min_transfer_points ?? '100'"
                                    class="w-full rounded-xl border border-gray-200 px-4 py-3 outline-none transition focus:border-black" />
                            </div>
                            <div class="rounded-xl bg-amber-50 p-3">
                                <p class="text-sm text-amber-700">Note: Please make sure this phone number is registered in the Yarmook app.</p>
                            </div>
                        </div>
                        <button @click="handleTransfer" :disabled="loadingTransfer || !transferPhone || !transferPoints"
                            ...
                            <span v-if="loadingTransfer" class="h-5 w-5 border-2 border-white border-t-transparent rounded-full animate-spin"></span>
